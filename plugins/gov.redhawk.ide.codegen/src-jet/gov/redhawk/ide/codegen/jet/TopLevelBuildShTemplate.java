...
public class TopLevelBuildShTemplate
{
  ...
  protected final String TEXT_12 = " ; do" + NL + "        pushd $impl &> /dev/null" + NL + "        if [ -e build.sh ]; then" + NL + "            ./build.sh $*" + NL + "        elif [ -e reconf ]; then" + NL + "            ./reconf && ./configure && make" + NL + "        else" + NL + "            echo \"No build.sh found for $impl\"" + NL + "        fi" + NL + "        popd &> /dev/null" + NL + "    done" + NL + "fi";
