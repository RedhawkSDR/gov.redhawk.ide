...
public class FindByEditFeature extends AbstractCustomFeature {
	 ...
	 */
	// TODO: From Devin - set true only for FindByName, Service, and EventChannel, since only those are generated by a wizard?
	@Override
	public boolean canExecute(ICustomContext context) {
		...
		if (pes != null && pes.length == 1) {
			if (DUtil.getBusinessObject(pes[0]) instanceof FindByStub) {
				...
				return (findByStub.getNamingService() != null 
						|| DomainFinderType.SERVICENAME.equals(type) 
						|| DomainFinderType.SERVICETYPE.equals(type)
	...
	public void execute(ICustomContext context) {
		...
		final FindByStub findByStub = (FindByStub) DUtil.getBusinessObject(pe);
		if (findByStub.getNamingService() != null) {
			editFindByCorbaName(findByStub);
		} else if (findByStub.getDomainFinder() != null) { 
			DomainFinderType type = findByStub.getDomainFinder().getType();
			if (DomainFinderType.SERVICENAME.equals(type) 
					|| DomainFinderType.SERVICETYPE.equals(type)) {
				editFindByService(findByStub);
	...
	private void editFindByCorbaName(final FindByStub findByStub) {
		// Find By CORBA Name
		...
		final List<String> usesPortNames = (page.getModel().getUsesPortNames() != null) ? page.getModel().getUsesPortNames()
			: null;
		final List<String> providesPortNames = (page.getModel().getProvidesPortNames() != null) ? page.getModel().getProvidesPortNames()
			: null;
		...
		stack.execute(new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				updatePorts(findByStub, usesPortNames, providesPortNames);
			}
		});
	}

	// TODO ports
	private void editFindByService(final FindByStub findByStub) {
		// Find By Service
	...
	private static void updatePorts(final FindByStub findByStub, final List<String> usesPortNames, final List<String> providesPortNames) {
		if (usesPortNames != null) {
			List<UsesPortStub> portsToDelete = new ArrayList<UsesPortStub>();
			for (UsesPortStub uses : findByStub.getUses()) {
				if (usesPortNames.contains(uses.getName())) {
					usesPortNames.remove(uses.getName());
				} else {
		...
		if (providesPortNames != null) {
			...
			for (ProvidesPortStub provides : findByStub.getProvides()) {
				if (providesPortNames.contains(provides.getName())) {
					providesPortNames.remove(provides.getName());
				} else {
