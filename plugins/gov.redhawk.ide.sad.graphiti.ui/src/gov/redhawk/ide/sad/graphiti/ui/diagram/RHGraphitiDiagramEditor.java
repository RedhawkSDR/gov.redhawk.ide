...
public class RHGraphitiDiagramEditor extends DiagramEditor {
									...
									GroupEditPart retVal = new GroupEditPart((PaletteGroup) model) {
										...
										protected EditPart createChild(Object model) {
											if ("Text".equals(model)) {
												...
												setPaletteFilter(label.getPaletteFilter());
												getPaletteFilter().setPaletteBehavior(RHGraphitiPaletteBehavior.this);
												((RHToolBehaviorProvider) RHGraphitiDiagramEditor.this.getDiagramTypeProvider().getCurrentToolBehaviorProvider()).setFilter(getPaletteFilter());
												return label;
											}
	...
	private boolean shouldAdd(ResourceDesc desc) {
		if (desc instanceof ComponentDesc) {
			...
			return ComponentType.RESOURCE.getLiteral().equalsIgnoreCase(type);
		} else {
			return false;
		}
	}
	...
	public void setPaletteFilter(RHGraphitiPaletteFilter paletteFilter) {
