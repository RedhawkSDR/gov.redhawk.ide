...
public class DiagramUtil {
	...
	public final static int LOLLIPOP_ELLIPSE_DIAMETER = 10;
	...
	public final static int START_ORDER_ELLIPSE_DIAMETER = 15;
	...
	public static ContainerShape findContainerShapeParentWithProperty(Shape shape, String propertyValue){
		...
		if(shape instanceof ContainerShape && DiagramUtil.isPropertyElementType(shape, propertyValue)){
			return (ContainerShape)shape;
		}
		if(DiagramUtil.isPropertyElementType(shape.getContainer(), propertyValue)){
	...
	public static ContainerShape findContainerShapeParentWithProperty(PictogramElement pe, String propertyValue){
		...
		if(peContainer instanceof ContainerShape){
			ContainerShape outerContainerShape = DiagramUtil.findContainerShapeParentWithProperty((ContainerShape)peContainer, propertyValue);
	...
	public static int getMinimumWidth(final String outerTitle, final String innerTitle, final EList<ProvidesPortStub> providesPortStubs, final EList<UsesPortStub> usesPortStubs, Diagram diagram){
		...
		innerTitleWidth = innerTitleDimension.getWidth() + INTERFACE_SHAPE_WIDTH + INNER_CONTAINER_SHAPE_TITLE_HORIZONTAL_PADDING + DiagramUtil.START_ORDER_ELLIPSE_DIAMETER + DiagramUtil.START_ORDER_ELLIPSE_LEFT_PADDING + DiagramUtil.START_ORDER_ELLIPSE_RIGHT_PADDING;
...
//	}
	...
	public static ContainerShape addOuterRectangle(ContainerShape targetContainerShape, String text, Object businessObject, 
		...
		Graphiti.getPeService().setPropertyValue(outerContainerShape, DiagramUtil.SHAPE_TYPE, DiagramUtil.SHAPE_outerContainerShape);
	...
	/**
	 * Return ContainerShape representing component start order ellipse
	 * @param innerContainerShape
	 * @param diagram
	 * @param sadComponentInstantiation
	 * @return
	 */
	public static ContainerShape addStartOrderEllipse(ContainerShape innerContainerShape, Diagram diagram, SadComponentInstantiation sadComponentInstantiation){
		...
		Graphiti.getPeService().setPropertyValue(startOrderEllipse, GA_TYPE, GA_startOrderEllipse);
		Graphiti.getGaLayoutService().setSize(startOrderEllipse, START_ORDER_ELLIPSE_DIAMETER, START_ORDER_ELLIPSE_DIAMETER);

		//port text
		Shape providesPortTextShape  = Graphiti.getCreateService().createShape(startOrderEllipseShape, false);
		Text startOrderText = Graphiti.getCreateService().createText(providesPortTextShape, sadComponentInstantiation.getStartOrder().toString());
		Graphiti.getPeService().setPropertyValue(startOrderText, GA_TYPE, GA_startOrderText);
	...
	public static void addLollipop(ContainerShape outerContainerShape, Diagram diagram, Object anchorBusinessObject, IFeatureProvider featureProvider){
		...
		lollipopEllipse.setStyle(StyleUtil.getStyleForLollipopEllipse(diagram));
		...
		Graphiti.getGaLayoutService().setLocationAndSize(lollipopLine, LOLLIPOP_ELLIPSE_DIAMETER, LOLLIPOP_ELLIPSE_DIAMETER/2, INTERFACE_SHAPE_WIDTH - LOLLIPOP_ELLIPSE_DIAMETER, 1);
	...
	public static void addProvidesPorts(ContainerShape outerContainerShape, Diagram diagram, EList<ProvidesPortStub> providesPortStubs, IFeatureProvider featureProvider){
		...
		Graphiti.getGaLayoutService().setLocationAndSize(providesPortsRectangle, DiagramUtil.PROVIDES_PORTS_LEFT_PADDING, DiagramUtil.PORTS_CONTAINER_SHAPE_TOP_PADDING, DiagramUtil.PORT_SHAPE_WIDTH + providesPortNameLength, providesPortStubs.size()*(DiagramUtil.PORT_SHAPE_HEIGHT));
	...
	public static void addUsesPorts(ContainerShape outerContainerShape, Diagram diagram, EList<UsesPortStub> usesPortStubs, IFeatureProvider featureProvider){
		...
		Graphiti.getGaLayoutService().setSize(usesPortsRectangle, PORT_SHAPE_WIDTH + usesPortNameLength + PORT_NAME_HORIZONTAL_PADDING, usesPortStubs.size()*(PORT_SHAPE_HEIGHT));
	...
	/**
	 * Executes layout for outerContainerShape.
	 * @param context
	 * @param pe
	 * @param outerTitle
	 * @param innerTitle
	 ...
	 * @param usesPortStubs
	 */
	public static void layoutOuterContainerShape(PictogramElement pe, final String outerTitle, final String innerTitle, 
				final EList<ProvidesPortStub> providesPortStubs, final EList<UsesPortStub> usesPortStubs){
		...
		Ellipse startOrderEllipse = null;
		
		//get shape being laid out
        ContainerShape containerShape = (ContainerShape) pe;
        RoundedRectangle outerRoundedRectangle = (RoundedRectangle)containerShape.getGraphicsAlgorithm();
        Diagram diagram = Graphiti.getPeService().getDiagramForPictogramElement(pe);
		
        // height
		int minimumHeight = DiagramUtil.getMinimumHeight(providesPortStubs, usesPortStubs);
        if (outerRoundedRectangle.getHeight() < minimumHeight) {
        	outerRoundedRectangle.setHeight(minimumHeight);
        }

        // width
        int minimumWidth = DiagramUtil.getMinimumWidth(outerTitle,innerTitle, providesPortStubs, usesPortStubs, diagram);
		...
		List<PropertyContainer> children = DiagramUtil.collectPropertyContainerChildren(containerShape);
		for(PropertyContainer pc: children){
			if(DiagramUtil.isPropertyElementType(pc, DiagramUtil.GA_innerRoundedRectangle)){
			...
			}else if(DiagramUtil.isPropertyElementType(pc, DiagramUtil.GA_startOrderEllipse)){
				startOrderEllipse = (Ellipse)pc;
			}
		}
		...
		gaLayoutService.setLocationAndSize(outerRoundedRectangleText, DiagramUtil.INNER_CONTAINER_SHAPE_HORIZONTAL_LEFT_PADDING+DiagramUtil.ICON_IMAGE_LENGTH+4, 0, containerWidth-(DiagramUtil.INNER_CONTAINER_SHAPE_HORIZONTAL_LEFT_PADDING+DiagramUtil.ICON_IMAGE_LENGTH+4), 20);
		...
		int innerContainerShapeWidth = containerWidth-DiagramUtil.INNER_CONTAINER_SHAPE_HORIZONTAL_PADDING*2 - DiagramUtil.PROVIDES_PORTS_LEFT_PADDING;
		int innerContainerShapeHeight = containerHeight-DiagramUtil.INNER_CONTAINER_SHAPE_TOP_PADDING;
		...
		IDimension innerRoundedRectangleTextSize = GraphitiUi.getUiLayoutService().calculateTextSize(innerRoundedRectangleText.getValue(), StyleUtil.getInnerTitleFont(Graphiti.getPeService().getDiagramForPictogramElement(containerShape)));
		int xForImage = (innerRoundedRectangle.getWidth()-(innerRoundedRectangleTextSize.getWidth()+DiagramUtil.ICON_IMAGE_LENGTH+5))/2;
		gaLayoutService.setLocationAndSize(innerRoundedRectangleImage, xForImage, DiagramUtil.INNER_ROUNDED_RECTANGLE_TEXT_TOP_PADDING, DiagramUtil.ICON_IMAGE_LENGTH, DiagramUtil.ICON_IMAGE_LENGTH);
		gaLayoutService.setLocationAndSize(innerRoundedRectangleText, xForImage+DiagramUtil.ICON_IMAGE_LENGTH+5, DiagramUtil.INNER_ROUNDED_RECTANGLE_TEXT_TOP_PADDING, innerRoundedRectangleTextSize.getWidth()+10, innerRoundedRectangleTextSize.getHeight());
		innerRoundedRectangleLine.getPoints().get(1).setX(innerContainerShapeWidth);
		
		//start order ellipse
		gaLayoutService.setLocation(startOrderEllipse, innerContainerShapeWidth - (DiagramUtil.START_ORDER_ELLIPSE_DIAMETER + DiagramUtil.START_ORDER_ELLIPSE_RIGHT_PADDING), DiagramUtil.START_ORDER_ELLIPSE_TOP_PADDING);
	...
	/**
	 * Iterates through all outer container shapes in the diagram and for all that link to SadComponentInstantiation update
	 * the Start Order Ellipse and Start Order Text based on the start order stored in the underlying model.
	 * @param diagram
	 */
	public static void organizeDiagramStartOrder(Diagram diagram){
		
		//find all of our diagram elements
		for(Shape shape: DiagramUtil.collectShapeChildren(diagram)){
			if(DiagramUtil.isPropertyElementType(shape, DiagramUtil.SHAPE_outerContainerShape)){
				Object businessObject = DiagramUtil.getBusinessObject(shape);
				if(businessObject != null && businessObject instanceof SadComponentInstantiation){
					SadComponentInstantiation sadComponentInstantiation = (SadComponentInstantiation)DiagramUtil.getBusinessObject(shape);
					List<PropertyContainer> children = DiagramUtil.collectPropertyContainerChildren(shape);
					for(PropertyContainer pc: children){
						if(DiagramUtil.isPropertyElementType(pc, DiagramUtil.GA_startOrderText)){
							Text startOrderText = (Text)pc;
							//update text
							startOrderText.setValue(sadComponentInstantiation.getStartOrder().toString());
							...
							Ellipse startOrderEllipse = (Ellipse)pc;
							//if start order zero (assembly controller), then use special style
							if(BigInteger.ZERO.compareTo(sadComponentInstantiation.getStartOrder()) == 0){
								startOrderEllipse.setStyle(StyleUtil.getStyleForStartOrderAssemblyControllerEllipse(diagram));
							}else{
