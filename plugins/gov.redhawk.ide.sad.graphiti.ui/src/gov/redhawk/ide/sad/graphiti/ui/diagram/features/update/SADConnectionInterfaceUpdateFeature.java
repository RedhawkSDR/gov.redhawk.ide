...
public class SADConnectionInterfaceUpdateFeature extends AbstractUpdateFeature {
	 ...
	 * @return
	 */
	public Reason internalUpdate(Connection connectionPE, SadConnectInterface connectInterface, 
			IFeatureProvider featureProvider, boolean performUpdate) {
		
		boolean updateStatus = false;

		// imgConnectionDecorator
		ConnectionDecorator imgConnectionDecorator = (ConnectionDecorator)DUtil.findFirstPropertyContainer(
				connectionPE, SADConnectInterfacePattern.SHAPE_imgConnectionDecorator);
		// textConnectionDecorator
		ConnectionDecorator textConnectionDecorator = (ConnectionDecorator)DUtil.findFirstPropertyContainer(
						connectionPE, SADConnectInterfacePattern.SHAPE_textConnectionDecorator);
		...
		if (connectInterface.getSource() != null && connectInterface.getTarget() != null) {
			...
			if ((!compatibleConnection || !uniqueConnection) && 
					(imgConnectionDecorator == null || textConnectionDecorator == null)) {
				if (performUpdate) {
					updateStatus = true;
