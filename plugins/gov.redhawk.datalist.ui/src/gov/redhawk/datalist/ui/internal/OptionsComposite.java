...
public abstract class OptionsComposite {
			...
			public void widgetSelected(final SelectionEvent e) {
				if (isIndefinite()) {
					...
					if (isRunning) {
						...
						setSettings(columnsCombo.getText(), Double.valueOf(samplesTxt.getText()), captureCombo.getText());
						startAcquire();
					}
				} else {
					isRunning = true;
				...
				if (isDouble(value)) {
					...
					if (method == CaptureMethod.INDEFINITELY) {
						...
						if (val > Integer.MAX_VALUE) {
							showDecoration(dec, settings.getProcessType() + " must be less than or equal to " + Integer.MAX_VALUE + ".");
	...
	public boolean isIndefinite() {
		return CaptureMethod.stringToValue(settings.getProcessType()).equals(CaptureMethod.INDEFINITELY);
	}
