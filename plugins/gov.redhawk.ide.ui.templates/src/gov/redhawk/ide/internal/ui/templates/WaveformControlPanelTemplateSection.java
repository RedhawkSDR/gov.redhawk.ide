 ...
 * 
 */
public class WaveformControlPanelTemplateSection extends BaseControlPanelTemplate implements ITemplateSection {

	/**
	 * Constructor for MultiPageEditorTemplate.
	 */
	public WaveformControlPanelTemplateSection() {
		setPageCount(1);
		createOptions();
	}
	...
	public String getSectionId() {
		final boolean multiPage = getBooleanOption("useMultiPageEditor");
		if (multiPage) {
			return "multiPageWaveformControlPanel";
		} else {
			return "waveformControlPanel"; //$NON-NLS-1$
		}
	}
	...
	public IPluginReference[] getDependencies(final String schemaVersion) {
		if (schemaVersion != null) {
			final IPluginReference[] dep = new IPluginReference[7]; // SUPPRESS CHECKSTYLE MagicNumber
			...
			dep[5] = new PluginReference("gov.redhawk.sca.util", null, 0); // SUPPRESS CHECKSTYLE MagicNumber //$NON-NLS-1$
			dep[6] = new PluginReference("org.eclipse.ui.ide", null, 0); // SUPPRESS CHECKSTYLE MagicNumber //$NON-NLS-1$
			return dep;
		}
	...
	private void createOptions() {
		// first page
		addOption("useMultiPageEditor", "&Multi Page Control Panel", false, 0);
		...
		addOption("editorClassName", //$NON-NLS-1$
		        "&Editor Class Name:", "SampleWaveformControlPanel", //$NON-NLS-1$
		        0).setRequired(true);
		addOption("contributorClassName", //$NON-NLS-1$
		        "&Editor Contributor Class Name:", "SampleWaveformControlPanelContributor", //$NON-NLS-1$
		        0).setRequired(true);
		addOption("editorName", //$NON-NLS-1$
		        "&Control Panel Name:", "Sample Waveform Control Panel", 0).setRequired(true);

		addOption("contentTypeNname", //$NON-NLS-1$
		        "&Content Type Name:", "Sample Waveform Content Type", 0).setRequired(false);
		final SdrRoot sdr = SdrUiPlugin.getDefault().getTargetSdrRoot();
		sdr.load(null);

		final WaveformsContainer waveforms = sdr.getWaveformsContainer();

		final List<String[]> waveformChoices = new ArrayList<String[]>();
		for (final SoftwareAssembly sad : waveforms.getWaveforms()) {
			String name = sad.getName();
			if (name == null || name.length() == 0) {
				name = sad.eResource().getURI().lastSegment();
			}
			waveformChoices.add(new String[] { sad.getId(), name });
		}
		if (waveformChoices.size() > 0) {
			addOption("contentTypeProfileId", //$NON-NLS-1$
			        "&Waveform:", waveformChoices.toArray(new String[waveformChoices.size()][]), waveformChoices.get(0)[0], 0).setRequired(true);
		}
	...
	public void addPages(final Wizard wizard) {
		...
		page.setTitle("Sample Waveform Control Panel");
	 ...
	 * (non-Javadoc)
	 * 
	 * @seeorg.eclipse.pde.internal.ui.wizards.templates.PDETemplateSection# formatPackageName(java.lang.String)
	 */
	@Override
