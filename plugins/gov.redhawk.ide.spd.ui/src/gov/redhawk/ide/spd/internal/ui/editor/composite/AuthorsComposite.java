...
import gov.redhawk.ide.spd.internal.ui.editor.provider.OverviewAdapterFactory;
import gov.redhawk.ide.spd.internal.ui.parts.AuthorsPart;
...
public class AuthorsComposite extends Composite {
	...
	public AuthorsComposite(final Composite parent, final int style, final FormToolkit toolkit,
	...
	private void createAuthorsArea(final Composite client, final FormToolkit toolkit, final IActionBars actionBars) {
		...
		this.authorsPart.getTableViewer().addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(final SelectionChangedEvent event) {
				getEditAuthorButton().setEnabled(
				        AuthorsComposite.this.authorsPart.isEnabled() && !event.getSelection().isEmpty());
			}
		});
		...
		getAuthorsViewer().addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(final SelectionChangedEvent event) {
				getRemoveAuthorButton().setEnabled(
				        AuthorsComposite.this.authorsPart.isEnabled() && !event.getSelection().isEmpty());
			}
		});
	}
