...
public class SnapshotWizardPage extends WizardPage {
    ...
    public SnapshotWizardPage(String pageName, ImageDescriptor titleImage, String [] supportedTypes,
    ...
    public SnapshotWizardPage(String pageName, ImageDescriptor titleImage, String [] supportedTypes,
    		String [] processMethods, long samples) {
    ...
    public void createControl(Composite main) {
        ...
        context.bindValue(WidgetProperties.text().observeDelayed(500, captureCombo),
        		PojoObservables.observeValue(settings, "captureType"));
        
        final Text samplesTxt = new Text(parent, SWT.BORDER);
        samplesTxt.setLayoutData(
        ...
        validateSamples.setBeforeSetValidator(new IValidator() {
			...
			public IStatus validate(Object value) {
				if (value instanceof Double) {
					IDataReceiver.CaptureMethod method = 
							IDataReceiver.CaptureMethod.stringToValue(settings.getCaptureType());
					if (method == IDataReceiver.CaptureMethod.INDEFINITELY) {
							return ValidationStatus.ok();
					}
					if (((Double) value).doubleValue() <= 0) {
						return ValidationStatus.error(settings.getCaptureType() 
					...
					if (method == IDataReceiver.CaptureMethod.NUMBER) {
						...
						if (val > Long.MAX_VALUE) {
							return ValidationStatus.error(settings.getCaptureType() 
									+ " must less than or equal to " + Long.MAX_VALUE);
						}
						if ((val - (long) val) > 0) {
							return ValidationStatus.error(settings.getCaptureType() 
					...
					return ValidationStatus.error(
							"The Number of Samples must be a positive number");
				}
			}
		});
        final Binding samplesBinding = context.bindValue(
        		WidgetProperties.text(SWT.Modify).observeDelayed(500, samplesTxt),
        ...
        captureCombo.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
            	...
            	IDataReceiver.CaptureMethod method = 
            			IDataReceiver.CaptureMethod.stringToValue(captureCombo.getText());
            	if (method == IDataReceiver.CaptureMethod.INDEFINITELY) {
            		...
            		samplesTxt.setEnabled(false);
            		return;
            	} else if (method == IDataReceiver.CaptureMethod.CLOCK_TIME 
            			|| method == IDataReceiver.CaptureMethod.SAMPLE_TIME) {
        ...
        fileTypeCombo.setLayoutData(
        		GridDataFactory.fillDefaults().grab(true, false).span(2, 1).create());
        fileTypeCombo.setItems(settings.getSupportedTypes());
        context.bindValue(WidgetProperties.text().observeDelayed(500, fileTypeCombo),
        		PojoObservables.observeValue(settings, "fileType"));
        ...
        context.bindValue(WidgetProperties.selection().observeDelayed(500, workspaceCheck),
        		PojoObservables.observeValue(settings, "saveToWorkspace"));
        ...
        fileFinder.setLayoutData(
        		GridDataFactory.fillDefaults().grab(true, true).span(3, 2).create());
        ...
        searchFileSystem.setLayoutData(
        		GridDataFactory.fillDefaults().grab(true, true).span(3, 2).create());
        final Group searchWorkbench = makeWorkbenchTree(fileFinder);
        searchWorkbench.setLayoutData(
        		GridDataFactory.fillDefaults().grab(true, true).span(3, 2).create());
    ...
    private Group makeFileSystemSave(Composite parent) {
        ...
        fileNameTxt.setLayoutData(
        		GridDataFactory.fillDefaults().grab(true, false).span(1, 1).create());
        
        context.bindValue(WidgetProperties.text(SWT.Modify).observeDelayed(500, fileNameTxt),
        		PojoObservables.observeValue(settings, "fileName"));
        // the browse button
    ...
    private Group makeWorkbenchTree(Composite parent) {
    	...
    	final TreeViewer workbenchTree = new TreeViewer(searchWorkbench, 
    	...
    	workbenchTree.getControl().setLayoutData(
    			GridDataFactory.fillDefaults().grab(true, true).span(3, 1).hint(
    					SWT.DEFAULT, 150).create());
    	...
    	newFolderItem.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
    			...
    			InputDialog newFolder = new InputDialog(workbenchTree.getTree().getShell(),
    					"New Folder", "Please enter the name of the folder to be placed in\n"
    			...
    			if (result == 0) {
    				...
    				if (filePathAbs.isDirectory()) {
    					MessageDialog.openError(workbenchTree.getTree().getShell(), "Error",
    	...
    	deleteFileItem.addSelectionListener(new SelectionAdapter() {
    		@Override
            public void widgetSelected(SelectionEvent e) {
    			MessageBox warning = new MessageBox(workbenchTree.getControl().getShell(), 
    					SWT.YES | SWT.NO);
    			...
    			if ((settings.getFileName() != null || settings.getFileName() != "") 
    					&& temp.endsWith(settings.getFileName())) {
    			...
    			try {
					...
					resource.delete(IResource.FORCE | IResource.KEEP_HISTORY 
							| IResource.ALWAYS_DELETE_PROJECT_CONTENT, null);
    			...
    			if (!worked) {
    				MessageBox failedMessage = new MessageBox(workbenchTree.getControl().getShell(),
    						SWT.ERROR | SWT.OK);
