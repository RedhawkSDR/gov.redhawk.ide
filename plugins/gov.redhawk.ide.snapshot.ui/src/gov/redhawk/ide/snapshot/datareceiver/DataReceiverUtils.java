...
public final class DataReceiverUtils {
	...
	public static boolean isSRIChanged(StreamSRI sri1, StreamSRI sri2) {
		...
		} else if (sri1.xstart != sri2.xstart || sri1.xdelta != sri2.xdelta || sri1.xunits != sri2.xunits) {
			match = false;
		} else if (sri1.ystart != sri2.ystart || sri1.ydelta != sri2.ydelta || sri1.yunits != sri2.yunits) {
			match = false;
		} else if (sri1.hversion != sri2.hversion || sri1.subsize != sri2.subsize || sri1.mode != sri2.mode) {
			match = false;
		} else if (sri1.blocking != sri2.blocking || !sri1.streamID.equals(sri2.streamID) || sri1.keywords.length != sri2.keywords.length) {
			match = false;
		} else {
			int i = 0;
			while (i < sri1.keywords.length && match) {
				if (!sri1.keywords[i].id.equals(sri2.keywords[i].id) || !sri1.keywords[i].value.equals(sri2.keywords[i].value)) {
					match = false;
				}
				i++;
			}
